...
GraphicsNodeSocket::GraphicsNodeSocket(Socket socket, QGraphicsItem *parent)
    : QGraphicsItem(parent), _highlight(false), _socket(socket),
      _socket_type(socket.port.lock()->direction),
      _pen_circle(PEN_COLOR_CIRCLE), _pen_unconnected_circle(PEN_COLOR_CIRCLE),
      _pen_text(PEN_COLOR_TEXT),
      _brush_circle((_socket_type == Port::Direction::IN) ? BRUSH_COLOR_SINK
                                                          : BRUSH_COLOR_SOURCE),
      _text(new EditableLabel()), _delete_button(TinyButton::forbidden(this)) {

  auto label_font = _text->font();
...
void GraphicsNodeSocket::notifyPositionChange() {
  ...
  case Port::Direction::IN:
    for (const auto &e : _edges)
      e->set_stop(mapToScene(0, 0));
    break;
  case Port::Direction::OUT:
    for (const auto &e : _edges)
      e->set_start(mapToScene(0, 0));
    break;
  }
}
