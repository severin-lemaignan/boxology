...
void GraphicsNodeView::wheelEvent(QWheelEvent *event) {
...
void GraphicsNodeView::mousePressEvent(QMouseEvent *event) {
    switch (event->button()) {
...
void GraphicsNodeView::leftMouseButtonRelease(QMouseEvent *event) {
    if (_drag_event) {
...
void GraphicsNodeView::mouseMoveEvent(QMouseEvent *event) {
...
bool GraphicsNodeView::can_accept_edge(GraphicsNodeSocket *sock) {
    if (sock && _drag_event &&
        ((sock->is_sink() &&
          _drag_event->mode == EdgeDragEvent::move_to_sink) ||
         (sock->is_sink() &&
...
GraphicsNodeSocket *GraphicsNodeView::socket_at(QPoint pos) {
