...
GraphicsNode::GraphicsNode(NodePtr node, QGraphicsObject *parent)
    : QGraphicsObject(parent), _node(node), _changed(false),
      _width(_node.lock()->width()), _height(_node.lock()->height()),
      _pen_default(QColor("#7F000000")), _pen_selected(QColor("#FFFF36A7"))
      // , _pen_selected(QColor("#FFFFA836"))
      // , _pen_selected(QColor("#FF00FF27"))
      ,
      _pen_sources(QColor("#FF000000")), _pen_sinks(QColor("#FF000000")),
      _brush_title(QColor("#E3212121")), _brush_background(QColor("#E31a1a1a")),
      _brush_sources(QColor("#FFFF7700")), _brush_sinks(QColor("#FF0077FF")),
  ...
  connect(_new_sink_btn, &TinyButton::triggered, this, &GraphicsNode::add_sink);
  ...
  if (source == _sources.end()) {
    ...
    if (sink == _sinks.end()) {
      qWarning() << "Trying to connect to port "
                 << QString::fromStdString(port->name) << " of node " << _title
...
void GraphicsNode::refreshNode() {
  ...
  auto color = QColor(QString::fromStdString(LABEL_COLORS.at(node->label())));
  color.setAlpha(120);
  setColors(color);
  ...
  if (_central_proxy != nullptr) {
    ...
    if (wgt) {
      ...
      if (sh.isValid()) {
        ...
        if (sz.width() > 0) {
          min_width =
              std::max(qreal(sz.width()) + 2.0 * _lr_padding, min_width);
        } else {
          min_width =
...
// Code to create a sub-architecture on double-clicking a node
...
//    }
//
//    auto topwindow = dynamic_cast<MainWindow
...
void GraphicsNode::mouseDoubleClickEvent(QGraphicsSceneMouseEvent *event) {
  // Show the dialog when the graphics object is double-clicked
