...
MainWindow::MainWindow()
    : _root_arch(new Architecture), _active_arch(_root_arch.get()),
      ui(new Ui::MainWindow), _view(nullptr), _root_scene(nullptr),
...
void MainWindow::spawnInitialNodes() {
  ...
  node2->name("Node 2");
  auto p2 =
      node2->createPort({"input", Port::Direction::IN, InterfaceType::UNKNOWN});
  auto gNode2 = _root_scene->add(node2);
  gNode2->setPos(30 + gNode->width() * 2, 170);

  node2->createPort({"output", Port::Direction::OUT, InterfaceType::UNKNOWN});
...
void MainWindow::on_actionAdd_node_triggered() {
  ...
  node->name("Node");
  node->createPort({"input", Port::Direction::IN, InterfaceType::UNKNOWN});
  node->createPort({"output", Port::Direction::OUT, InterfaceType::UNKNOWN});
...
void MainWindow::on_actionToJsonAs_triggered() {
  auto newPath = QFileDialog::getSaveFileName(this, "Save architecture to json",
                                              _jsonPath, "JSON file (*.json)");
